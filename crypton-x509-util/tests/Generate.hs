...
createKey (OpenSSLKey keyType keyName keyBits) =
    case keyType of
        ...
        DSA ->
            readProcess "openssl" ["dsaparam", "-genkey", show keyBits, "-out", keyName] ""
        ECDSA -> undefined

createPub (OpenSSLKey keyType keyName _) pubName =
    case keyType of
        RSA ->
            readProcess "openssl" ["rsa", "-in", keyName, "-pubout", "-out", pubName] ""
        DSA ->
            readProcess "openssl" ["dsa", "-in", keyName, "-pubout", "-out", pubName] ""
        _ -> undefined
