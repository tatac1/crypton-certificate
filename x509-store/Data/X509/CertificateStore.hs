...
makeCertificateStore = CertificateStore . foldl' accumulate M.empty
  where
    accumulate m x509 = M.insert (certSubjectDN $ getCertificate x509) x509 m

-- | Find a certificate using the subject distinguished name
findCertificate
    :: DistinguishedName -> CertificateStore -> Maybe SignedCertificate
findCertificate dn store = lookupIn store
  where
    lookupIn (CertificateStore m) = M.lookup dn m
...
readCertificateStore path = do
    ...
    wrapStore
        <$> (if isDir then makeDirStore else if isFile then makeFileStore else return [])
  where
  ...
  where
    ...
    getDirContents =
        E.catch
            ...
            emptyPaths
      where
        emptyPaths :: E.IOException -> IO [FilePath]
        emptyPaths _ = return []
