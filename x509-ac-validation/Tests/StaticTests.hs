...
testV2FormBaseCertID = do
  let v2form = V2Form
        { v2fromIssuerName = [AltDirectoryName (mkDn "Test AA")]
        , v2fromBaseCertificateID = Just (IssuerSerial [] 1 Nothing)  -- Not allowed!
        , v2fromObjectDigestInfo = Nothing
        }
  let aci = AttributeCertificateInfo
        { aciVersion = 1
        , aciHolder = Holder (Just (IssuerSerial [] 1 Nothing)) Nothing Nothing
...
testV2FormObjDigest = do
  ...
  let v2form = V2Form
        { v2fromIssuerName = [AltDirectoryName (mkDn "Test AA")]
        , v2fromBaseCertificateID = Nothing
        , v2fromObjectDigestInfo = Just objDigest  -- Not allowed!
        }
  let aci = AttributeCertificateInfo
        { aciVersion = 1
        , aciHolder = Holder (Just (IssuerSerial [] 1 Nothing)) Nothing Nothing
...
testIssuerNotSingleDN = do
  ...
  let v2form = V2Form
        { v2fromIssuerName = [AltDirectoryName (mkDn "AA1"), AltDirectoryName (mkDn "AA2")]
        , v2fromBaseCertificateID = Nothing
        , v2fromObjectDigestInfo = Nothing
        }
  let aci = AttributeCertificateInfo
...
--------------------------------------------------------------------------------
-- TCG Platform Certificate Tests (AC-TCG-*)
--------------------------------------------------------------------------------

tcgTests :: TestTree
tcgTests =
  testGroup
    "TCG Platform Certificate (AC-TCG-*)"
    [ testCase "AC-TCG-9.1: Valid TCG Platform Certificate" testValidTCGPlatform,
      testCase "AC-TCG-9.2: Holder baseCertificateID Missing (rejected)" testTCGHolderBaseCertMissing,
      testCase "AC-TCG-9.3: Holder entityName Present (rejected)" testTCGHolderEntityName,
      testCase "AC-TCG-9.4: Holder objectDigestInfo Present (rejected)" testTCGHolderObjDigest
    ]

-- | AC-TCG-9.1: Valid TCG Platform Certificate should pass
testValidTCGPlatform :: Assertion
testValidTCGPlatform = do
  let aci = AttributeCertificateInfo
        { aciVersion = 1
        , aciHolder = Holder (Just (IssuerSerial [AltDirectoryName (mkDn "EK Issuer")] 1 Nothing)) Nothing Nothing
        , aciIssuer = AttCertIssuerV2 $ V2Form [AltDirectoryName (mkDn "Platform CA")] Nothing Nothing
        , aciSignature = SignatureALG HashSHA256 PubKeyALG_RSA
        , aciSerialNumber = 1
        , aciValidity = mkTestValidityPeriod defaultValidityStart defaultValidityEnd
        , aciAttributes = Attributes []
...
-- | AC-TCG-9.2: TCG Platform Cert missing baseCertificateID should be rejected
testTCGHolderBaseCertMissing :: Assertion
testTCGHolderBaseCertMissing = do
  let aci = AttributeCertificateInfo
        { aciVersion = 1
        , aciHolder = Holder Nothing (Just [AltDirectoryName (mkDn "Entity")]) Nothing  -- Using entityName instead!
        , aciIssuer = AttCertIssuerV2 $ V2Form [AltDirectoryName (mkDn "Platform CA")] Nothing Nothing
        , aciSignature = SignatureALG HashSHA256 PubKeyALG_RSA
        , aciSerialNumber = 1
        , aciValidity = mkTestValidityPeriod defaultValidityStart defaultValidityEnd
  ...
  let result = validateTCGPlatform aci
  assertBool "Should have TCGHolderBaseCertIDRequired error" $
    TCGHolderBaseCertIDRequired `elem` vrErrors result

-- | AC-TCG-9.3: TCG Platform Cert with entityName should be rejected
testTCGHolderEntityName :: Assertion
testTCGHolderEntityName = do
  let aci = AttributeCertificateInfo
        { aciVersion = 1
        , aciHolder = Holder
            (Just (IssuerSerial [] 1 Nothing))
            (Just [AltDirectoryName (mkDn "Entity")])  -- Not allowed in TCG!
            Nothing
        , aciIssuer = AttCertIssuerV2 $ V2Form [AltDirectoryName (mkDn "Platform CA")] Nothing Nothing
        , aciSignature = SignatureALG HashSHA256 PubKeyALG_RSA
        , aciSerialNumber = 1
  ...
  let result = validateTCGPlatform aci
  assertBool "Should have TCGHolderInvalidField error for entityName" $
    any isTCGInvalidField (vrErrors result)
  where
    isTCGInvalidField (TCGHolderInvalidField "entityName") = True
    isTCGInvalidField _ = False

-- | AC-TCG-9.4: TCG Platform Cert with objectDigestInfo should be rejected
testTCGHolderObjDigest :: Assertion
testTCGHolderObjDigest = do
  let objDigest = ObjectDigestInfo
        { odiObjectType = OIDPublicKey
        , odiOtherObjectTypeID = Nothing
        , odiDigestAlgorithm = SignatureALG HashSHA256 PubKeyALG_RSA
  ...
  let aci = AttributeCertificateInfo
        ...
        , aciHolder = Holder
            (Just (IssuerSerial [] 1 Nothing))
            Nothing
            (Just objDigest)  -- Not allowed in TCG!
        , aciIssuer = AttCertIssuerV2 $ V2Form [AltDirectoryName (mkDn "Platform CA")] Nothing Nothing
        , aciSignature = SignatureALG HashSHA256 PubKeyALG_RSA
        , aciSerialNumber = 1
        , aciValidity = mkTestValidityPeriod defaultValidityStart defaultValidityEnd
  ...
  let result = validateTCGPlatform aci
  assertBool "Should have TCGHolderInvalidField error for objectDigestInfo" $
    any isTCGInvalidField (vrErrors result)
  where
    isTCGInvalidField (TCGHolderInvalidField "objectDigestInfo") = True
    isTCGInvalidField _ = False
