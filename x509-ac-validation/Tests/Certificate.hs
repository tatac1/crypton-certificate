...
-- | Default extensions for AA (Attribute Authority) certificates.
-- An AA needs keyCertSign for signing ACs.
...
-- | Builds an AA (Attribute Authority) certificate for signing ACs.
mkAA
  :: Integer
  -- ^ Serial number
  -> String
...
-- | Holds together an AC and the AA that signed it.
...
-- | Create a test holder from an AA certificate.
...
-- | Create a test issuer from an AA certificate.
...
-- | Sign an AttributeCertificateInfo to create a SignedExact AC.
signAttributeCertificate
  :: Alg pub priv
  -> priv
  -> AttributeCertificateInfo
...
-- | Create a complete test AC signed by an AA.
mkTestAC
  :: Pair pub priv
...
-- | Create a test CRL signed by an issuer.
mkTestCRL
  :: Pair pub priv
  -- ^ Issuer certificate/key pair (AA or CA)
  -> DateTime
  -- ^ This update time
  -> Maybe DateTime
  -- ^ Next update time (None means CRL doesn't expire)
  -> [RevokedCertificate]
