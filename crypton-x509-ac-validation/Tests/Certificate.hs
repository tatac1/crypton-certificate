...
-- signed AC structure ('SignedAttributeCertificate' from "Data.X509AC").
...
mkTestHolder aaCert =
    let cert = signedObject (getSigned aaCert)
        ...
        issuerSerial = IssuerSerial [AltNameDN issuerDn] serial Nothing
...
mkTestIssuer aaCert =
    let cert = signedObject (getSigned aaCert)
     ...
     in AttCertIssuerV2 $ V2Form [AltNameDN dn] Nothing Nothing
