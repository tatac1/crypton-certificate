...
--
-- This generator produces a singleton list containing an AltNameDN
...
tests =
    testGroup
        ...
        [ testGroup
            ...
            , testProperty
                "v2Form with valid issuerName passes"
                prop_v2Form_valid_issuerName_passes
            , testProperty "serial number validation" prop_serial_number_validation
            , testProperty
                "holder with at least one field passes"
                prop_holder_with_field_passes
            , testProperty "holder with all fields empty fails" prop_holder_empty_fails
            , testProperty
                "v2Form baseCertificateID present fails"
            ...
            , testProperty
                ...
                prop_unknown_critical_ext_rejected
            , testProperty "known critical extension passes" prop_known_critical_ext_passes
            , testProperty
                "non-critical unknown extension passes"
                prop_noncritical_unknown_ext_passes
...
genValidV2FormIssuer = do
    dn <- suchThat arbitrary (\(DistinguishedName l) -> not (null l))
    let v2 = V2Form [AltNameDN dn] Nothing Nothing
...
prop_v2Form_baseCertID_fails =
    forAll arbitrary $ \is ->
        forAll (suchThat arbitrary (\(DistinguishedName l) -> not (null l))) $ \dn ->
            forAll genValidHolder $ \holder -> do
                let v2 = V2Form [AltNameDN dn] (Just is) Nothing
...
prop_v2Form_objDigest_fails =
    forAll arbitrary $ \odi ->
        forAll (suchThat arbitrary (\(DistinguishedName l) -> not (null l))) $ \dn ->
            forAll genValidHolder $ \holder -> do
                let v2 = V2Form [AltNameDN dn] Nothing (Just odi)
                let issuer = AttCertIssuerV2 v2
                let aci = makeTestACI holder issuer 1
                let result = validateRFC5755Profile aci
...
makeTestACIWithExtensions exts =
    let holder = Holder (Just $ IssuerSerial [AltNameDNS "test"] 1 Nothing) Nothing Nothing
        dn =
            DistinguishedName
                [(getObjectID DnCommonName, ASN1CharacterString UTF8 "Test Issuer")]
        issuer = AttCertIssuerV2 $ V2Form [AltNameDN dn] Nothing Nothing
