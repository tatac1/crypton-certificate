...
data PubKey
    ...
    | -- | EC public key
      PubKeyEC PubKeyEC
    | -- | X25519 public key
      PubKeyX25519 X25519.PublicKey
    | -- | X448 public key
      PubKeyX448 X448.PublicKey
    | -- | Ed25519 public key
...
instance ASN1Object PubKey where
    fromASN1 (Start Sequence : Start Sequence : OID pkalg : xs)
        ...
        | pkalg == getObjectID PubKeyALG_DSA =
            case xs of
                Start Sequence : IntVal p : IntVal q : IntVal g : End Sequence : End Sequence
                    : BitString bits
                    : End Sequence
                    : xs2 ->
                    decodeASN1Err
      ...
      where
        decodeASN1Err format bits xs2 f =
            case decodeASN1' BER (bitArrayGetData bits) of
                Left err ->
                    Left
                        ("fromASN1: X509.PubKey " ++ format ++ " bitarray cannot be parsed: " ++ show err)
                Right s -> case f s of
                    Left err -> Left err
            ...
            CryptoFailed err ->
                Left
                    ( "fromASN1: X509.PubKey "
                        ++ format
                        ++ " bitarray contains an invalid public key: "
                        ++ show err
                    )
...
encodePK key = asn1Container Sequence (encodeInner key)
  where
    ...
    encodeInner (PubKeyRSA pubkey) =
      ...
      where
        bits = encodeASN1' DER $ rsaPubToASN1 pubkey []
...
rsaPubFromASN1 (Start Sequence : IntVal smodulus : IntVal pubexp : End Sequence : xs) =
  ...
  where
    pub =
        RSA.PublicKey
            { RSA.public_size = numBytes modulus
...
toPositive int
  ...
  where
    ...
    bytesOfInt n = if testBit (head nints) 7 then nints else 0xff : nints
      where
        nints = reverse $ plusOne $ reverse $ map complement $ bytesOfUInt (abs n)
        plusOne [] = [1]
