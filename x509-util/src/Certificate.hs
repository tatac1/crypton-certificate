...
formatValidity (start, end) = p start ++ " to " ++ p end
  where
    p t = timePrint ("YYYY-MM-DD H:MI:S" :: String) t
...
showDN (X509.DistinguishedName dn) = mapM_ toStr dn
  where
    toStr (oid, cs@(ASN1CharacterString e t)) =
        putStrLn ("  " ++ key ++ ": " ++ value)
      where
        key = show oid
...
showExts es@(Extensions (Just exts)) = do
    ...
    showKnownExtension
        "basic-constraint"
        (X509.extensionGetE es :: Maybe (Either String X509.ExtBasicConstraints))
    showKnownExtension
        "key-usage"
        (X509.extensionGetE es :: Maybe (Either String X509.ExtKeyUsage))
    showKnownExtension
        "extended-key-usage"
        (X509.extensionGetE es :: Maybe (Either String X509.ExtExtendedKeyUsage))
    showKnownExtension
        "subject-key-id"
        (X509.extensionGetE es :: Maybe (Either String X509.ExtSubjectKeyId))
    showKnownExtension
        "subject-alt-name"
        (X509.extensionGetE es :: Maybe (Either String X509.ExtSubjectAltName))
    showKnownExtension
        "authority-key-id"
  ...
  where
    showExt er = do
        putStrLn
            ("  OID:  " ++ show (extRawOID er) ++ " critical: " ++ show (extRawCritical er))
        either (\e -> putStrLn $ "ASN1 decoding failed: " ++ e) (showASN1 8) $
            tryExtRawASN1 er
...
showDSAKey (DSA.PrivateKey params privnum) =
    unlines
...
readSignedObject file = do
    content <- B.readFile file
    return $
        either error (map (X509.decodeSignedObject . pemContent)) $
            pemParseBS content

doCertMain opts files = do
    when (Help `elem` opts) $ do
        putStrLn $
            usageInfo "usage: x509-util cert [options] <certificates>" optionsCert
        exitSuccess
...
optionsCert =
    ...
    , Option
        []
        ["validation-host"]
        (ReqArg ValidationHost "host")
        "validation host use for validation"
