...
import Data.X509 (SignatureALG(..), objectToSignedExact, objectToSignedExactF, Extensions(..), PubKeyALG(..), HashALG(..), DistinguishedName(..))
...
createPlatformCertificate config components tpmInfo = do
  return $ createPlatformCertificateSync config components tpmInfo

-- | Synchronous version of createPlatformCertificate for easier testing
createPlatformCertificateSync :: PlatformConfiguration 
                              -> [ComponentIdentifier]
                              -> TPMInfo
                              -> Either String SignedPlatformCertificate
createPlatformCertificateSync config components tpmInfo = do
  -- Create the basic certificate info structure
  certInfo <- buildPlatformCertificateInfo config components tpmInfo
...
buildPlatformCertificateInfo :: PlatformConfiguration 
                             -> [ComponentIdentifier]
                             -> TPMInfo
                             -> Either String PlatformCertificateInfo
buildPlatformCertificateInfo config components tpmInfo = do
  ...
  buildPlatformCertificateInfoWithValidity config components tpmInfo validity

-- | Helper function to build PlatformCertificateInfo with custom validity period
buildPlatformCertificateInfoWithValidity :: PlatformConfiguration 
                                         -> [ComponentIdentifier]
                                         -> TPMInfo
                                         -> AttCertValidityPeriod
                                         -> Either String PlatformCertificateInfo
buildPlatformCertificateInfoWithValidity config components tpmInfo validity = do
  
  -- Create a simple holder (entity name based)
  let holder = HolderEntityName []
  
  -- Create a simple issuer (V2 form)
  let issuer = AttCertIssuerV2 (V2Form [] Nothing Nothing)
  
  -- Create attributes from config, components, and TPM info
  attrs <- buildAttributesFromConfig config components tpmInfo
  
  return $ PlatformCertificateInfo
...
buildAttributesFromConfig :: PlatformConfiguration 
                          -> [ComponentIdentifier] 
                          -> TPMInfo
...
mkPlatformCertificate config components tpmInfo validity auth (algS, _pubKey, privKey) = do
  let validityPeriod = AttCertValidityPeriod (fst validity) (snd validity)
  case buildPlatformCertificateInfoWithValidity config components tpmInfo validityPeriod of
...
createDeltaPlatformCertificate baseCert componentDeltas changeRecords = do
  ...
  let baseCertRef = BasePlatformCertificateRef
        { bpcrIssuer = issuerDN
  ...
  let platformDelta = PlatformConfigurationDelta
  ...
  let _deltaConfig = DeltaPlatformConfiguration
...
validateChainContinuity chain = 
  let baseRef = ccBaseCertificate chain
   ...
   in concat
        [ ["Duplicate serial numbers in certificate chain" | hasDuplicates],
          ["Delta certificate serial conflicts with base certificate" | hasBaseConflict]
        ]
  where
    nub :: Eq a => [a] -> [a]
    nub [] = []
