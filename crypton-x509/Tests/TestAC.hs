...
instance Arbitrary Holder where
    ...
    arbitrary = do
        ...
        if all
            isNothing
            [ fmap (const ()) mBaseCertID
            , fmap (const ()) mEntityName
            , fmap (const ()) mObjDigestInfo
            ]
            then
                oneof
                    ...
                    , Holder
                        <$> pure Nothing
                        <*> (Just <$> listOf1NonEmpty arbitrary)
                        <*> pure Nothing
...
instance Arbitrary IetfAttrSyntax where
    arbitrary =
        (IetfAttrSyntax . Just <$> listOf1NonEmpty arbitrary)
            <*> listOf1NonEmpty arbitrary
...
tests =
    testGroup
        ...
        [ testGroup
            ...
            , testProperty
                "Attributes"
                (property_unmarshall_marshall_id :: Attributes -> Bool)
            , testProperty
                "AttributeCertificateInfo"
                (property_unmarshall_marshall_id :: AttributeCertificateInfo -> Bool)
            , testProperty
                "AttributeRaw"
                (property_unmarshall_marshall_id :: Raw.AttributeRaw -> Bool)
            ]
        , testGroup
            ...
            , testProperty
                "Service Auth Info"
                (prop_attribute_roundtrip :: [Attr_SvceAuthInfo] -> Property)
            , testProperty
                "Access Identity"
                (prop_attribute_roundtrip :: [Attr_AccessIdentity] -> Property)
            , testProperty
                "Charging Identity"
                (prop_attribute_roundtrip :: [Attr_ChargingIdentity] -> Property)
            , testProperty "Group" (prop_attribute_roundtrip :: [Attr_Group] -> Property)
            , testProperty
                "Clearance"
                (prop_attribute_roundtrip :: [Attr_Clearance] -> Property)
            ]
        ]
