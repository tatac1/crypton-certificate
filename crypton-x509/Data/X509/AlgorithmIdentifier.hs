...
sigOID sig =
    maybe (error ("unknown OID for " ++ show sig)) fst $
        find ((==) sig . snd) sig_table
...
instance ASN1Object SignatureALG where
    ...
    toASN1 signatureAlg@(SignatureALG hashAlg PubKeyALG_RSAPSS) = \xs ->
        Start Sequence
            : OID [1, 2, 840, 113549, 1, 1, 10]
            : Start Sequence
            : Start (Container Context 0)
            : Start Sequence
            : OID (sigOID signatureAlg)
            : End Sequence
    ...
    -- RFC 5758 §3.2: ECDSA parameters MUST be absent; RFC 3279 §2.2.2: DSA parameters MUST be absent
